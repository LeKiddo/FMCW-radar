...
class uartParserSDK():
    def __init__(self, type='(Legacy) 2D People Counting'):
        ...
        if (self.ifdm):
            ...
            self.xyz_PC = np.zeros(1000 * self.maxPoints * 3).reshape(
                (1000, self.maxPoints, 3))
    ...
    def polar2Cart(self):
        ...
        for n in range(0, self.numDetectedObj):
            ...
            self.xyz_PC[self.frameNum % 1000, n, 0] = self.pcBufPing[0, n]
            self.xyz_PC[self.frameNum % 1000, n, 1] = self.pcBufPing[1, n]
            self.xyz_PC[self.frameNum % 1000, n, 2] = 0
    ...
    def tlvHeader(self, data):
        ...
        while (1):
            try:
                ...
                # bad data, return
                print("\n failed in tlvHeader, bad data, returned \n")
                self.fail = 1
                return data
    ...
    def sdk3xTLVHeader(self, dataIn):
        ...
        while (1):
            try:
                ...
                # bad data, return
                print("\n failed in sdk3xTLVHeader bad data \n")
                self.fail = 1
                return dataIn
            if (magic != self.magicWord):
                # wrong magic word, increment pointer by 1 and try again
    ...
    def sdk3xPCHeader(self, dataIn):
        ...
        while (1):
            try:
                ...
                # bad data, return
                print("\n failed in sdk3xPCHeader bad data \n")
                self.fail = 1
                return dataIn
            if (magic != self.magicWord):
                # wrong magic word, increment pointer by 1 and try again
    ...
    def Capon3DHeader(self, dataIn):
        ...
        while (1):
            try:
                ...
                # bad data, return
                print("failed in Capon3DHeader, cannot Read Frame Header")
                # print(e)
                self.fail = 1
                return dataIn
            if (magic != self.magicWord):
        ...
        if (self.saveTextFile):
            if (self.frameNum % 1000 == 0):
                ...
                if (self.capon3D):
                    ...
                    for i in range(1000):
                        tfile.write(
                            'magic, version, packetLength, platform, frameNum, subFrameNum, chirpMargin, frameMargin, uartSentTime, trackProcessTime, numTLVs, checksum, datetime_\n')
                        for j in range(0, 12):
                            tfile.write(str(self.textStructCapon3D[i, 0, j, 0]))
                            tfile.write(',')
                            # print(str(self.textStructCapon3D[i,0,j,0]))
                        datetime_=str(datetime.datetime.now().hour) + ":" + str(datetime.datetime.now().minute) + ':' + str(
                            datetime.datetime.now().second) + str(datetime.datetime.now().microsecond)+','

                        tfile.write(datetime_)
                        tfile.write('\n')
                ...
                if (self.ifdm):
                    ...
                    for i in range(1000):
                        tfile.write(
                            'magic, version, platform, timestamp, packetLength, frameNum, subFrameNum, chirpMargin, frameMargin, uartSentTime, trackProcessTime, numTLVs, checksum,datetime_\n')
                        for j in range(13):
                            tfile.write(str(self.textStruct2D[i, 0, j, 0]))
                            tfile.write(',')
                        datetime_ = str(datetime.datetime.now().hour) + ":" + str(
                            datetime.datetime.now().minute) + ':' + str(
                            datetime.datetime.now().second) + str(datetime.datetime.now().microsecond) + ','
                        tfile.write(datetime_)
                        tfile.write('\n')
                        tfile.write('range, azim, doppler, snr,x,y,z\n')
                        for j in range(np.count_nonzero(self.textStruct2D[i, 1, :, 0])):
                            for k in range(4):
                                tfile.write(str(self.textStruct2D[i, 1, j, k]))
                                tfile.write(',')
                            for o in range(3):
                                tfile.write(str(self.xyz_PC[i, j, o]))
                                tfile.write(',')
                            tfile.write('\n')
                    ...
                    if self.saveNumTxt == 1:
                        ...
                        print("self.stop_xlsx =", self.stop_xlsx, "self.saveNumTxt", self.saveNumTxt)
                                  ...
                                  self.saveNumTxt % 3) == 1 and self.stop_xlsx != 1):  # TODO evolving integer for stop_xlsx and save in new xlsx when (self.saveNumTxt % 8) == 1 because excel files have a max of possible rows ~1Million
